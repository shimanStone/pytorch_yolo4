...
class YoloDataset(Dataset):
    ...
    def __getitem__(self, item):
        """返回image, box[c_x, c_y, w, h] in normalization(0-1)"""
        index = item % self.length

        if self.mosaic:
            if self.rand() < 0.5 and self.epoch_now < self.epoch_length * self.mosaic_ratio:
                lines = sample(self.annotation_lines, 3)  # 在序列中随机提取k个片段
    ...
    def get_random_data_with_mosaic(self, annotation_line, input_shape, jitter=0.3, hue=0.1, sat=0.7, val=0.4):
        h, w = input_shape
        min_offset_x, min_offset_y = self.rand(0.3,0.7), self.rand(0.3,0.7)
        ...
        for line in annotation_line:
            ...
            # 对图像进行缩放并进行长宽扭曲
            new_arr = iw / ih * self.rand(1-jitter, 1+jitter) / self.rand(1-jitter, 1+jitter)
